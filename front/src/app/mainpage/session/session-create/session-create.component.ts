...
export class SessionCreateComponent implements OnInit {
	...
	ngOnInit() {
		...
		let endDate = new Date(startDate.getTime() + 7200000);
	...
	validateDate(control: AbstractControl): ValidationErrors | null {
		let startDate = new Date(control.get('startDate')?.value as string);
		let endDate = new Date(control.get('endDate')?.value as string);
	...
	onSubmit() {
		let startDate = new Date(this.sessionForm.value.startDate as string);
		let endDate = new Date(this.sessionForm.value.endDate as string);
		...
		this.sessionService.newSession(createSession).subscribe(session => {
...
const TP = [{name: 'Kafka', id: "1d234cef65487"}, {name: 'Scrapping', id: "1d4578cab"},
