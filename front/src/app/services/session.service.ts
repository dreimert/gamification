...
export class SessionService {
	...
	public getAvailableSessions(): Observable<Session[] | TeacherSession[]> {
		return new Observable<Session[]>(subscriber => {
			this.http.get<Session[] | TeacherSession[]>(this.root + 'available').subscribe((sessions: Session[] | TeacherSession[]) => {
	...
	public joinSession(session: Session, password: string): Observable<any> {
		return new Observable<Session>(subscriber => {
			this.http.post(this.root + session.id + '/join', {id: session.id, password}).subscribe({
				next: (data: any) => {
	...
	public deleteSession(session: Session | TeacherSession): Observable<any> {
	...
	public endSession(session: Session | TeacherSession): Observable<any> {
		return new Observable<any>(subscriber => {
