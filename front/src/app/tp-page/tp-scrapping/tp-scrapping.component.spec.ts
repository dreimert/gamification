...
describe('TpScrappingComponent', () => {
  ...
  beforeEach(async () => {
    ...
    tpScrappingService = {
        getLvl1Code(token:string):Observable<lvl1Code>{
            return of<lvl1Code>({
                teacherName:'teacherName',
            });
        },
        getLvl2Course(token:string):Observable<lvl2Course>{
            return of<lvl2Course>({
                courseCode:'string',
            });
        },
        getLvl3BookInfo(token:string):Observable<lvl3Res>{
            return of<lvl3Res>({});
        },
        getLvl4TeacherInfo(token:string):Observable<lvl4Res>{
            return of<lvl4Res>({
                p1Name: 'string',
                p2Initials: 'string',
                p3Phone: 'string',
        ...
        verifyCode(token:string):Observable<verifyPassCode>{
      ...
      providers: [
        {
            ...
            // useFactory: () => jasmine.createSpyObj(baseName: "MatDialogRef")
        }
      ]
    })
    .compileComponents();
